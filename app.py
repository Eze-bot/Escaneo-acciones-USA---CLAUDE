...
# ── Leer API Key desde st.secrets ANTES de importar config ───────────────────
try:
    _api_key_from_secrets = st.secrets.get("ANTHROPIC_API_KEY", "")
except Exception:
    _api_key_from_secrets = ""

import config
if _api_key_from_secrets:
    config.ANTHROPIC_API_KEY = _api_key_from_secrets
...
with st.sidebar:
    ...
    if config.ANTHROPIC_API_KEY:
        ...
        ak = st.text_input("Ingresá tu API Key", type="password", placeholder="sk-ant-...")
        if ak:
    ...
    uploaded_file = st.file_uploader("Reemplazar tickers.csv (opcional)", type=["csv"])
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    config.MIN_PRICE = st.number_input("Precio mín (USD)", value=float(config.MIN_PRICE), min_value=0.0, step=1.0)
    config.MAX_PRICE = st.number_input("Precio máx (USD)", value=float(config.MAX_PRICE), min_value=0.0, step=10.0)
    st.markdown('</div>', unsafe_allow_html=True)

    # GAP
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("**📊 Filtros de GAP**")
    st.caption("⚠️ Fuera del horario de mercado el GAP puede ser 0. Podés bajar el mínimo a 0.")
    config.MIN_GAP_PCT = st.number_input("GAP mín (%)", value=float(config.MIN_GAP_PCT), min_value=0.0, step=0.1, format="%.1f")
    config.MAX_GAP_PCT = st.number_input("GAP máx (%)", value=float(config.MAX_GAP_PCT), min_value=0.0, step=1.0, format="%.1f")
    st.markdown('</div>', unsafe_allow_html=True)

    # Riesgo
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("**🛡️ Gestión de Riesgo**")
    config.STOP_LOSS_PCT     = st.number_input("Stop Loss (%)",      value=float(config.STOP_LOSS_PCT),     min_value=0.1, step=0.1, format="%.1f")
    config.TARGET_PROFIT_PCT = st.number_input("Target ganancia (%)", value=float(config.TARGET_PROFIT_PCT), min_value=0.1, step=0.1, format="%.1f")
    ...
    debug_mode = st.checkbox("Mostrar detalle de rechazados", value=True)
    max_tickers = st.number_input("Máx tickers a analizar (0 = todos)", value=50, min_value=0, step=10)
    st.caption("Usá un número bajo para pruebas rápidas")
    st.markdown('</div>', unsafe_allow_html=True)
...
# ── Carga de tickers ──────────────────────────────────────────────────────────
ticker_records = []
src = uploaded_file if uploaded_file else os.path.join(os.path.dirname(os.path.abspath(__file__)), "tickers.csv")
...
if max_tickers and max_tickers > 0:
    ticker_records = ticker_records[:max_tickers]
...
if ticker_records:
    ...
    st.warning("⚠️ No hay tickers cargados.")
...
with c2:
    st.caption("Analiza los tickers seleccionados y muestra el TOP 6")
...
if run_scan and tickers:
    ...
    for i, symbol in enumerate(tickers):
        pct = (i + 1) / total
        bar.progress(pct, text=f"Analizando {symbol}... ({i+1}/{total})")
        status.caption(f"⏳ Procesando `{symbol}`")
        ...
        try:
            ...
            if not ok:
                rejected.append({
                    "symbol": symbol,
                    "razón":  reason,
                    "gap%":   round((premarket or {}).get("gap_pct", 0), 2),
                    "precio": round((premarket or {}).get("pre_price", 0), 2),
                    "volumen": (premarket or {}).get("pre_volume", 0),
                })
                continue
            ...
            if not signals:
                rejected.append({"symbol": symbol, "razón": "Sin datos para indicadores", "gap%": 0, "precio": 0, "volumen": 0})
                continue
            ...
            rejected.append({"symbol": symbol, "razón": f"Error: {str(e)[:60]}", "gap%": 0, "precio": 0, "volumen": 0})
    ...
    m4.metric("Seleccionados",   min(len(candidates), config.TOP_N))

    # ── Diagnóstico de rechazados (siempre visible si no hay resultados) ──────
    if debug_mode or not candidates:
        if rejected:
            st.markdown("### 🔬 Detalle de rechazados")
            df_rej = pd.DataFrame(rejected)
            st.dataframe(df_rej, use_container_width=True)

            # Mostrar distribución de razones
            if len(rejected) > 0:
                razones = pd.Series([r["razón"].split("(")[0].strip() for r in rejected])
                st.markdown("**Razones más frecuentes:**")
                for razon, cnt in razones.value_counts().head(5).items():
                    st.caption(f"• {razon}: **{cnt}** tickers")

    if not candidates:
        st.warning("""
        ⚠️ **Ningún ticker pasó los filtros.**
        
        Sugerencias para obtener resultados:
        - Bajá el **GAP mínimo a 0%** (especialmente fuera del horario de mercado)
        - Ampliá el rango de **precio** (ej: $1 - $1000)
        - Reducí el **volumen mínimo** en config
        - Ejecutá durante el **horario de mercado** (9:30 - 16:00 ET)
        """)
    else:
        ...
        for rank, res in enumerate(top6, 1):
            ...
            with st.container():
                ...
                with c2:
                    tipo = res["type"].upper()
                    ab = "badge-lev" if "APALANCADO" in tipo else "badge-etf" if "ETF" in tipo else "badge-stock"
                    tc = "badge-up" if res["trend"] == "ALCISTA" else "badge-down" if res["trend"] == "BAJISTA" else "badge-lat"
                    ...
                    <span class='badge' style='background:#0D2040;color:#90CAF9'>GAP {gs}{res['gap_pct']:.2f}%</span>
                    """, unsafe_allow_html=True)
                with c3:
                ...
                si = {"MUY ALCISTA": "🟢", "ALCISTA": "🟡", "NEUTRO": "⚪",
                      "BAJISTA": "🟠", "MUY BAJISTA": "🔴"}.get(res['sentiment_label'], "⚪")
                st.markdown(f"""
    ...
    st.error("❌ No hay tickers para analizar. Revisá los filtros de tipo en el sidebar.")
else:
    st.markdown("""
